...
def save_to_pcd(data_dir, pcd_dir, timestamp):
    ...
    depth = read_pfm(a)
    # 定义一个缩放因子，例如1000表示将米转换为毫米
    scale = 100
    # 将浮点数组乘以缩放因子，并转换为uint16类型
    depth_array_scaled = (depth * scale).astype(np.uint16)
    # 创建一个open3d.geometry.Image对象
...
'''
pcd_data = pypcd.PointCloud.from_path('D:\Work\AirSim\AirSimDataset\process_data\demo4\\test.pcd')
points = np.zeros([pcd_data.width, 4], dtype=np.float32)
points[:, 0] = pcd_data.pc_data['x'].copy()
points[:, 1] = pcd_data.pc_data['y'].copy()
points[:, 2] = pcd_data.pc_data['z'].copy()
...
'''
def main():
    root_dir = 'D:\Work\AirSim\AirSimDataset\process_data\\v1_0'
    #data_dirs = ['scene3/seq0','scene3/seq1','scene3/seq2','scene3/seq3','scene3/seq4','scene3/seq5','scene3/seq6']
    data_dirs = ['scene3/seq3']
    pcd_dir = 'pcd\\'
    for data_dir in data_dirs:
        os.makedirs(os.path.join(root_dir, data_dir, 'bin'))
        for pcd_file in os.listdir(os.path.join(root_dir, data_dir, pcd_dir)):
            ...
            bin_dir = os.path.join(root_dir, data_dir, 'bin\\')
            with open(os.path.join(bin_dir, timestamp+'.bin'), 'wb') as f_bin:
